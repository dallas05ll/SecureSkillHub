...
| `roles/MEMORY_MANAGER.md` | MemM role: cross-role memory auditor, 9 sub-agents, 4 protocols (LOAD/WRITE/EVOLVE/HEALTH) | MemM |
...
| `scripts/enrich/detect_plugin_repos.py` | Detect Claude Code plugin manifests in skill repos (batch scans unscanned repos) | WS6 (AXM) |
| **`scripts/secm/`** | | |
...
| `scripts/secm/secm_evolve.py` | SecM self-evolve loop — learns from FP audits, writes to SecM structured memory | SecM |
...
| `site/api/skills/` | ~11,098 individual skill JSON files |
| `site/api/skills/index.json` | Full skills index sorted by unified priority `max(stars, installs)` |
...
| `site/api/marketplace.json` | API copy of marketplace manifest (top 200 verified plugins) |
| **`site/api/indexes/`** | **Agent-access indexes (generated by scripts/build/build_indexes.py)** |
| `site/api/indexes/manifest.json` | All 11,098 skill IDs with key metadata |
...
| `site/api/indexes/verify-queue.json` | Prioritized verification queue by unified priority `max(stars, installs)` |
...
### `skills/` — Claude Code Plugin Skills (AXM-owned)

| Path | Brief |
|------|-------|
| `skills/browse/SKILL.md` | Browse skill: embedded catalog map for in-context navigation without fetching |
| `skills/search/SKILL.md` | Search skill: fetches search-index.json, keyword matching against names + tags |
| `skills/install/SKILL.md` | Install skill: fetches skill JSON by ID, shows verification + install + safety warnings |

### `.claude-plugin/` — Plugin Manifest (AXM-owned)

| Path | Brief |
|------|-------|
| `.claude-plugin/plugin.json` | Plugin manifest: name="secureskillhub", version, author, skill list |
| `.claude-plugin/marketplace.json` | Marketplace registry: top 200 verified plugins (generated by `build_marketplace.py`) |

### `packages/` — npm/TypeScript Packages

| Path | Brief |
|------|-------|
| `packages/secureskillhub-mcp/` | MCP server package: `@secureskillhub/mcp-server`, 5 tools (AXM-owned) |
| `packages/secureskillhub-mcp/SKILL.md` | MCP server SKILL.md for Claude Code integration |
| `packages/secureskillhub-mcp/README.md` | MCP server public README (npm/GitHub facing) |
| `packages/secureskillhub-mcp/src/` | MCP server TypeScript source |
| `packages/secureskillhub-mcp/package.json` | npm manifest for `@secureskillhub/mcp-server` |
...
| **`api/src/routes/v2/`** | **v2 dynamic endpoints (Phase 2, 2026-03-05)** |
| `api/src/routes/v2/search.ts` | v2 search endpoint: full-text search with in-memory cache |
| `api/src/routes/v2/skill.ts` | v2 skill detail endpoint: `/v2/skill/:id` |
| `api/src/routes/v2/packages.ts` | v2 packages list endpoint |
| `api/src/routes/v2/stats.ts` | v2 stats endpoint |
| **`api/src/lib/`** | |
| `api/src/lib/github-oauth.ts` | GitHub OAuth helpers |
| `api/src/lib/install-command.ts` | Install command string generator |
| `api/src/lib/rate-limit.ts` | Rate limiting via Workers Cache API (100 req/min/IP) |
| `api/src/lib/search-cache.ts` | In-memory search-index cache with 10-min TTL + stale-while-revalidate |
| `api/src/lib/tier.ts` | Skill tier classification helpers |
| `api/src/lib/types.ts` | Shared TypeScript types |
| **`api/src/middleware/`** | |
| `api/src/middleware/auth.ts` | Auth middleware: validates bearer tokens |
| `api/src/middleware/rate-limit.ts` | Rate-limit middleware applied to v2 routes |
| **`api/src/db/`** | |
...
| Path | Brief |
|------|-------|
| `data/skills/` | **Source of truth** — 11,098 individual skill JSON files |
| `data/packages/` | 75 source package definition JSON files |
| `data/discovered/` | Raw crawl batch files (input to scripts/crawl/process_discovered.py) |
| `data/scan-reports/` | ~5,516 per-skill scan report directories |
